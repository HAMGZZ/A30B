...
void Logger::Send(Level level, const char *message, float var1, float var2)
{
    ...
    if(var1 == 0 && var2 == 0)
    {
        Serial.printf("LOG <<%d>>[%s %0.1lf][%s][%s]::  %s\n\r", millis(), PROG_NAME, VERSION, name, enum_to_string(level), message);
    }

    else if(var2 == 0)
    {
        Serial.printf("LOG <<%d>>[%s %0.1lf][%s][%s]::  %s %lf\n\r", millis(), PROG_NAME, VERSION, name, enum_to_string(level), message, var1);
    }
    else
    {
        Serial.printf("LOG <<%d>>[%s %0.1lf][%s][%s]::  %s %lf %lf\n\r", millis(), PROG_NAME, VERSION, name, enum_to_string(level), message, var1, var2);
    }
...
void Logger::Send(Level level, const char *message, bool var1)
{
    ...
    Serial.printf("LOG <<%d>>[%s %0.1lf][%s][%s]::  %s %s\n\r", millis(), PROG_NAME, VERSION, name, enum_to_string(level), message, var1 ? "True" : "False");
...
void Logger::Send(Level level, char const *message, char *message2)
{
    ...
    Serial.printf("LOG <<%d>>[%s %0.1lf][%s][%s]::  %s %s\n\r", millis(), PROG_NAME, VERSION, name, enum_to_string(level), message, message2);
